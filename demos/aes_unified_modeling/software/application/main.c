...
// CPU usage
#define CPU_USAGE_PERIOD_MS 150
...
int main()
{
	...
	float *cpu_usage = start_CPU_usage_measurement(CPU_USAGE_PERIOD_MS);

	for(int i = 0; i < 10; i++) {
		sleep(1);
		float user_cpu = cpu_usage[0];
		float kernel_cpu = cpu_usage[1];
		float idle_cpu = cpu_usage[2];

		printf("CPU usage: user = %f, kernel = %f, idle = %f\n", user_cpu, kernel_cpu, idle_cpu);
